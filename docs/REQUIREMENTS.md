...
- 요구사항: 사용자가 명시적으로 켠 경우, macOS menu bar status area에 Mac Hotspot Proxy Toggle 상태 항목을 표시함.
...
- 명세: menu bar companion은 별도 Swift binary로 제공함. `hotspot-proxy-toggle run`/`off`는 SSID, router IP, local path를 포함하지 않는 UI state JSON을 쓰고, companion은 이 파일을 watch해 상태만 표시함. UI state file이 아직 없거나 읽을 수 없을 때만 `hotspot-proxy-toggle status`를 fallback으로 호출함. menu top에는 `Mac Hotspot Proxy Toggle <version>` header를 표시함. 사용자가 메뉴에서 `Reconcile Now`를 선택한 경우에만 `hotspot-proxy-toggle run`을 호출함. 상태 문구는 notification의 hotspot proxy active, hotspot-but-proxy-unavailable, active-Wi-Fi-but-not-configured-hotspot 3상태와 맞추고, not-Wi-Fi/no-router와 error를 별도 상태로 표시함. Menu bar icon은 같은 휴대폰 핫스팟 glyph의 filled, outline, filled-slash 세 형태로 hotspot+proxy on, non-hotspot, hotspot+proxy off를 구분하며 기본 status item은 icon-only로 표시함. `Quit Mac Hotspot Proxy Toggle`은 `hotspot-proxy-toggle off`로 proxy setting을 끄고 helper/menu LaunchAgent를 unload함. Companion은 hotspot/proxy decision이나 macOS proxy write policy를 재구현하지 않음. Source installer는 `HOTSPOT_MENU_BAR=1`일 때만 companion LaunchAgent를 설치하고 시작함.
...
## R9: Finder 실행 가능한 App Bundle

- 요구사항: source install은 사용자가 Finder, Spotlight, Launchpad에서 실행할 수 있는 `MHP.app`을 설치함.
- 근거: `Quit Mac Hotspot Proxy Toggle`로 menu/helper LaunchAgent를 내린 뒤 terminal command를 기억하지 않아도 쉽게 다시 시작할 수 있어야 함.
- 방지 실패: menu bar companion을 끈 사용자가 재시작하려면 launchctl 또는 installer를 다시 실행해야 하는 상황을 막음.
- 명세: `MHP.app`은 기존 menu bar companion binary를 app bundle로 포장한 LSUIElement 앱임. 앱은 Dock icon을 표시하지 않고 menu bar item만 표시하며, display name은 `Mac Hotspot Proxy Toggle`임. App bundle은 핫스팟 프록시 켜짐 상태 glyph를 기반으로 한 `MHP.icns`를 포함함. Finder에서 앱을 실행하면 기존 helper 또는 polling LaunchAgent plist가 있는 경우 이를 다시 bootstrap/kickstart하고, 상태 menu를 표시함. App bundle은 hotspot/proxy decision이나 macOS proxy write policy를 재구현하지 않음. Source installer는 기존 설치 호환성을 위해 기본적으로 `~/Applications/MHP.app`을 설치하며, `HOTSPOT_APP=0`으로 생략할 수 있음.
