...
Status item은 title 옆에 template image icon을 표시함. Icon은 binary 내부에서 AppKit vector drawing으로 생성하며 외부 asset file에 의존하지 않음. macOS light/dark menu bar tint와 맞추기 위해 template image로 설정함.
