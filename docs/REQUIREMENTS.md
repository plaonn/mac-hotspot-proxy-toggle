...
- 명세: menu bar companion은 별도 Swift binary로 제공함. `hotspot-proxy-toggle run`/`off`는 SSID, router IP, local path를 포함하지 않는 UI state JSON을 쓰고, companion은 이 파일을 watch해 상태만 표시함. UI state file이 아직 없거나 읽을 수 없을 때만 `hotspot-proxy-toggle status`를 fallback으로 호출함. 사용자가 메뉴에서 `Reconcile Now`를 선택한 경우에만 `hotspot-proxy-toggle run`을 호출함. 상태 문구는 notification의 hotspot proxy active, hotspot-but-proxy-unavailable, active-Wi-Fi-but-not-configured-hotspot 3상태와 맞추고, not-Wi-Fi/no-router와 error를 별도 상태로 표시함. Menu bar icon은 같은 휴대폰 핫스팟 glyph의 filled, outline, filled-slash 세 형태로 hotspot+proxy on, non-hotspot, hotspot+proxy off를 구분하며 기본 status item은 icon-only로 표시함. `Quit MHP`는 `hotspot-proxy-toggle off`로 proxy setting을 끄고 helper/menu LaunchAgent를 unload함. Companion은 hotspot/proxy decision이나 macOS proxy write policy를 재구현하지 않음. Source installer는 `HOTSPOT_MENU_BAR=1`일 때만 companion LaunchAgent를 설치하고 시작함.
...
- 근거: `Quit MHP`로 menu/helper LaunchAgent를 내린 뒤 terminal command를 기억하지 않아도 쉽게 다시 시작할 수 있어야 함.
